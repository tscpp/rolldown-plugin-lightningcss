...
type TransformerInterface = {
  /**Provide a custom transformer instead of lightningcss, useful if you want to include CSS as-is */
  transformer?: (css: string) => string;
};

export interface LightningcssOptions<C extends CustomAtRules>
  extends
...
export function lightningcss<C extends CustomAtRules>(
  options: LightningcssOptions<C> = {},
): Plugin {
  return {
    name: "lightningcss",
    ...
    transform: {
      ...
      async handler(inputCode, id) {
        let magicString: RolldownMagicString;
        let finalMap: string | null = "";
        if (options.transformer) {
          const fileContents = await this.fs.readFile(id, { encoding: "utf8" });
          const code = options.transformer(fileContents.toString());
          magicString = new RolldownMagicString(
          ...
          magicString = new RolldownMagicString(draftCss, { filename: id });

          for (const dependency of bundled.dependencies!) {
            switch (dependency.type) {
              case "url": {
                if (
                  dependency.url.startsWith("data:") ||
                  dependency.url.startsWith("http:") ||
                  dependency.url.startsWith("https:")
                ) {
                  magicString.replace(dependency.placeholder, dependency.url);
                  break;
                }
                const resolved = await this.resolve(dependency.url, id, {
                  kind: "new-url",
                });
                if (!resolved) {
                  throw this.error(
                    `Unable to resolve '${dependency.url}'.`,
          ...
          finalMap = options?.sourceMap
            ? sourceMapRemapping(
                [
                  magicString.generateMap().toString(),
                  decoder.decode(bundled.map!),
                ],
                () => null,
              ).toString()
            : null;
      ...
      for (const [name, chunk] of Object.entries(bundle)) {
        if (
          ...
          const code = chunk.code.replaceAll(
            URL_PLACEHOLDER_REGEXP,
            (...args) => toRelative(this.getFileName(args[1]), chunk.fileName),
          );
